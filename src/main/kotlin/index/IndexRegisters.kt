...
class IndexRegisters(val dataBus: Bus, clk: Observable<Int>) {
    ...
    fun write() {
        ...
        if (log.isDebugEnabled) {
            log.debug(String.format("Wrote index register %d with %X", index, dataBus.value.toLong()))
        }
    }
    ...
    fun increment() {
        ...
        if (log.isDebugEnabled) {
            log.debug(String.format("Incremented index register %d. New value is %X", index, regs[index].readDirect().toLong()))
        }
    }
